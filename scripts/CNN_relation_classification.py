...
class Data(object):
    ...
    def _read_test_tsv(self, test_tsv):
        ...
        df = pd.read_csv(
            test_tsv, delimiter="\t", names=["id", "sentence"], converters={"id": lambda x: str(x)}
        )
        return df
    ...
    def _prepare_data_x_train(self, df):
        ...
        x_sents = [sequence for sequence in self._tokenizer.texts_to_sequences_generator(x_sents)]
    ...
    def _prepare_data_y(self, df):
        ...
        return self._label_smoothing(to_categorical(y_labels), self._label_smooth_frac)
    ...
    def _label_smoothing(self, array, label_smooth_frac):
        ...
        if label_smooth_frac:
            return array * (1 - label_smooth_frac) + (1 - array) * label_smooth_frac / (
                array.shape[1] - 1
            )
        else:
            return array
    ...
    def hard_voting_prediction(self, x):
        ...
        for model, _ in self._models:
            ...
            all_predictions += np.array(
                [np.eye(NUM_CLASSES)[n] for n in np.argmax(model_prediction, axis=1)]
            )
...
class KFoldValidator(object):
    ...
    def kfold_validation(self, k=10):
        ...
        for x_train, y_train, x_dev, y_dev in self._kfold_datasets(k):
            ...
            training_data = {"x": x_train, "y": y_train, "validation_data": [x_dev, y_dev]}
            params = {**training_data, **params}
            model.fit(**params)
            prediction = self._reverse_label_encoding(np.argmax(model.predict(x_dev), axis=1))
            true_labels = self._reverse_label_encoding(np.argmax(y_dev["primary_out"], axis=1))
    ...
    def _kfold_datasets(self, k):
        ...
        Args:
            k:  The number of folds
        """
        chunks_x = {input_key: np.array_split(data, k) for input_key, data in self._data_x.items()}
        chunks_y = {
            output_key: np.array_split(data, k) for output_key, data in self._data_y.items()
        }
        for fold in range(k):
            x_train = None
            y_train = None
            x_dev = {input_key: chunks[fold] for input_key, chunks in chunks_x.items()}
            y_dev = {output_key: chunks[fold] for output_key, chunks in chunks_y.items()}
            for i in range(k):
                if i != fold:
                    if x_train is None and y_train is None:
                        x_train = {input_key: chunks[i] for input_key, chunks in chunks_x.items()}
                        y_train = {output_key: chunks[i] for output_key, chunks in chunks_y.items()}
                    else:
                        x_train = {
                            input_key: np.vstack((x_train[input_key], chunks[i]))
                            for input_key, chunks in chunks_x.items()
                        }
                        y_train = {
                            output_key: np.vstack((y_train[output_key], chunks[i]))
                            for output_key, chunks in chunks_y.items()
                        }
...
def write_prediction_file(outpath, ids, labels):
    ...
    pred_array = np.array([[pred_id, pred_label] for pred_id, pred_label in zip(ids, labels)])
...
def relation_model(emb_num, emb_words, activation, loss, optimizer):
    ...
    distance_embeddings = Embedding(input_dim=400, output_dim=emb_num, input_length=200)
    emb_e1 = distance_embeddings(e1_input)
    emb_e2 = distance_embeddings(e2_input)

    # embedding layer for the whole sentence
    word_embeddings = Embedding(input_dim=5000, output_dim=emb_words, input_length=200)
    ...
    conv_three = Conv1D(filters=100, kernel_size=3, activation=activation, padding="same")(combined)
    conv_five = Conv1D(filters=100, kernel_size=5, activation=activation, padding="same")(combined)
    conv_seven = Conv1D(filters=100, kernel_size=7, activation=activation, padding="same")(combined)
...
def relation_params(batch_size, class_weight=None):
    ...
    callbacks = [
        EarlyStopping(
            monitor="val_acc", patience=5, verbose=1, restore_best_weights=True, mode="max"
        )
    ]
    params = {
        "epochs": 300,
...
def main():
    ...
    my_data.prepare_data(
        train_path, test_path, dev_frac=0.1, max_len=200, tokenizer=tokenizer, label_smooth_frac=0.1
    )
    ...
    train_data = {
        "x_train": {
            "whole_sent_input": x_train,
            "e1_input": pos_e1_train,
            "e2_input": pos_e2_train,
        },
        "y_train": {"primary_out": y_train},
        "x_dev": {"whole_sent_input": x_dev, "e1_input": pos_e1_dev, "e2_input": pos_e2_dev},
        "y_dev": {"primary_out": y_dev},
    }

    # models and fitting params
    class_weight = {
        cl: 1 - list(np.argmax(y_train, axis=1)).count(cl) / len(y_train)
        for cl in set(np.argmax(y_train, axis=1))
    }
    ...
    models = [
        (relation_model(**model_p), relation_params(**params_p))
        for model_p, params_p in models_function_params
    ]

    # ensemble predictions
    predictor = EnsemblePredictor(models, train_data)
    predictor.fit_models()
    y_dev_pred = predictor.soft_voting_prediction(
        {"whole_sent_input": x_dev, "e1_input": pos_e1_dev, "e2_input": pos_e2_dev}
    )
    y_test_pred = predictor.soft_voting_prediction(
        {"whole_sent_input": x_test, "e1_input": pos_e1_test, "e2_input": pos_e2_test}
    )
    ...
    # kfold_data = {
    #     "data_x": {"whole_sent_input": x_train, "e1_input": pos_e1_train, "e2_input": pos_e2_train},
    ...
    #     (relation_model(**test_parms[0]), relation_params(**test_parms[1])),
    #     kfold_data,
